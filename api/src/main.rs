...
async fn main() -> Result<()> {
    ...
    let database_client = {
        ...
        let options = {
            let mut options = MongoClientOptions::parse(uri)
                .await
                .context("failed to parse environment variable MONGO_URI")?;
    ...
    let settings = {
        let web_public_url = {
            ...
            url.parse().context(
                "failed to parse environment variable HOME_WEB_PUBLIC_URL",
            )?
        };
...
async fn recover(rejection: Rejection) -> Result<impl Reply, Infallible> {
    let (error, status_code) = if rejection.is_not_found() {
        ...
        let error = ErrorRejection::from(error.to_string());
